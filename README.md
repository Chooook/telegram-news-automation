...
1. Клонируйте репозиторий:
   ```sh
   git clone https://github.com/pankeig/telegram-news-automation.git
   cd telegram-news-automation
   ```
2. Создайте файл `.env` по примеру ниже:
   ```env
   API_ID=ваш_api_id
   API_HASH=ваш_api_hash
   BOT_TOKEN=ваш_бот_токен
   DB_PASSWORD=your_password_here
   # и другие переменные, если нужно
   ```
3. Запустите проект:
   ```sh
...
## Автоматическая установка и перенос вашей базы

1. Подготовьте дамп вашей базы PostgreSQL:
   ```sh
   pg_dump -U bot_user -d telegram_bot_db > mydb_dump.sql
   ```
2. Скопируйте `mydb_dump.sql` и `.env` в папку с проектом (или на уровень выше).
3. Запустите автоматический инсталлятор:
   ```sh
   chmod +x install.sh
   ./install.sh
   ```
   Скрипт сам клонирует репозиторий, восстановит базу, соберёт и запустит всё в Docker!

## Структура docker-compose
- **db** — PostgreSQL 15, все данные сохраняются в volume `db_data`
- **bot** — ваш Telegram-бот, автоматически подключается к базе
...
## Важно
- Не забудьте настроить переменные в `.env` и `config.yml`.
- Все секреты должны храниться только в `.env` (НЕ коммитьте его в git).
- Для первого запуска база создастся автоматически.

---

**Вопросы и помощь:** [@pankeig](https://t.me/pankeig)
