...
async def init_db_pool():
    ...
    try:
        ...
        logger.info("Database pool initialized successfully with pgvector support.")
        return pool
        
    except Exception as e:
        logger.error(f"Ошибка при инициализации пула соединений с базой данных: {str(e)}")
        logger.error("Проверьте настройки подключения к базе данных и убедитесь, что:")
    ...
    try:
        async with pool.acquire() as conn:
            ...
            try:
                ...
                logger.warning(f"Could not create vector extension (it might already exist): {e}")
...
async def save_article(pool, title, link, description, source, tags, published=None):
    """
...
async def add_embedding(pool, article_id, embedding):
    ...
    try:
        ...
        if not isinstance(embedding, (list, tuple)):
            raise ValueError(f"Эмбеддинг должен быть списком чисел, получен: {type(embedding)}")
        
        # Убедимся, что все элементы - числа
        embedding_list = [float(x) for x in embedding]
        if len(embedding_list) != 384:
            raise ValueError(f"Эмбеддинг должен содержать 384 числа, получено: {len(embedding_list)}")
        ...
        logger.error(f"Ошибка при сохранении эмбеддинга для статьи {article_id}: {e}")
        raise
...
async def fix_existing_embeddings(pool):
    ...
    try:
        async with pool.acquire() as conn:
            ...
            logger.info(f"Найдено {count} эмбеддингов в неправильном формате, исправляем...")
...
async def find_similar_articles(pool, embedding, limit=5, start_date=None, end_date=None):
    """
    ...
    try:
        ...
        if start_date:
            param_count += 1
            query += f" AND n.published >= ${param_count}"
            params.append(start_date)
            
        if end_date:
            param_count += 1
            # Add one day to end_date to include the entire end day
            query += f" AND n.published < ${param_count}::date + interval '1 day'"
        ...
        logger.error(f"Error finding similar articles: {str(e)}", exc_info=True)
        return []
...
async def get_channels(pool):
    ...
    async with pool.acquire() as conn:
        return [row['username'] for row in await conn.fetch("SELECT username FROM channels")]

async def remove_channel(pool, username):
    """Removes a channel from the database."""
    async with pool.acquire() as conn:
        await conn.execute("DELETE FROM channels WHERE username = $1", username)
        return [row['username'] for row in await conn.fetch("SELECT username FROM channels")]
...
async def get_articles_by_date_range(pool, start_date, end_date):
    ...
    query = """
        ...
        FROM news 
        WHERE published >= $1::date 
        AND published <= ($2::date + interval '1 day')
...
async def ensure_vector_extension_exists():
    ...
    try:
        ...
        logger.info("Vector extension and database schema verified successfully.")
        
    except Exception as e:
        if "permission denied" in str(e).lower() or "must be superuser" in str(e).lower():
            logger.warning("Недостаточно прав для настройки расширения vector. Требуются права суперпользователя.")
            logger.warning("Пожалуйста, выполните вручную в psql: CREATE EXTENSION IF NOT EXISTS vector;")
        else:
